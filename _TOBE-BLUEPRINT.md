...
| 4.1 | Phân bổ batch theo chiến thuật (FEFO/LEFO/Nhập xưởng/Chỉ định batch) — 6 khối | D. Logic | ☑ Xong |
| 4.2 | Phân bổ theo luồng & nguồn lấy hàng (In-In / In-Ex BKD / In-Ex NKD) — 6 khối | D. Logic | ☑ Xong |
...
>
> Các ngưỡng trên là **ràng buộc vận hành thực tế** do Mondelez/kho áp đặt (ví dụ ngưỡng 100 pallet bắt nguồn từ việc tồn chỉ lên SAP khi đơn hoàn thành 100%). Chi tiết logic áp dụng từng tham số nằm ở các nghiệp vụ tương ứng (mục **4.1**, **4.2**, **5.1**, **5.3**). _(Truy vết: BR-031, BR-009, BR-015, BR-029, BR-025; BRULE-02.)_
...
**Mục đích:** Hiện trạng, việc "lấy ở kho nào trước, lấy chung những kho nào" nằm trong trí nhớ của nhân viên kho ("ưu tiên BKD2/BKD3, thiếu mới lấy BKD1"). Mục này khai báo **tường minh** quy tắc đó thành một bảng: với mỗi tổ hợp (luồng × kho nguồn ghi trong kế hoạch), liệt kê các **bậc lấy hàng** theo thứ tự, mỗi bậc gồm những kho được **xét chung khi chọn lô**. Nhờ vậy khi cần đổi quy tắc (thêm kho nguồn, đổi thứ tự dự phòng) thì chỉ sửa ở đây, không phải sửa hệ thống và không phải suy đoán từ cấu hình kho.
...
Mỗi dòng khai báo một bậc lấy hàng cho một tổ hợp (luồng × cụm kho nguồn):

| STT | Tên trường | Mô tả | Ví dụ |
|---|---|---|---|
| 1 | Luồng | In-In hoặc In-Ex | In-Ex |
| 2 | Cụm kho nguồn (theo kế hoạch) | Giá trị cột *Cụm kho nguồn* trong file kế hoạch mà bậc này áp dụng | BKD |
| 3 | Bậc lấy | Thứ tự xét (số nhỏ trước); cùng một (luồng × cụm kho nguồn) có thể có nhiều bậc | 1 |
| 4 | Các kho lấy ở bậc này | Danh sách kho của bậc — nếu nhiều kho thì **hệ thống xét chung lô của tất cả các kho đó** (so hạn dùng trên cả nhóm, không ưu tiên/không lấy cạn kho nào trước) rồi chọn theo chiến thuật | BKD2, BKD3 |

Cấu hình mặc định đề xuất theo hiện trạng vận hành:

| Luồng | Cụm kho nguồn (kế hoạch) | Bậc lấy | Các kho lấy ở bậc này |
|---|---|---|---|
| **In-In** | BKD | 1 | BKD2, BKD3 (xét chung) |
| **In-Ex** | BKD | 1 | BKD2, BKD3 (xét chung) |
| **In-Ex** | BKD | 2 (dự phòng) | BKD1 |
| **In-Ex** | NKD | 1 | NKD |

> **Kho đích bị loại khỏi nguồn dù cùng cụm:** kho đích của mỗi dòng lấy từ **cột *Kho đích* trong file kế hoạch** (mục **3.3**) — với In-In luôn là **BKD1**. Vì một kho không thể vừa là nguồn vừa là đích trong cùng một dòng, nên cụm nguồn của In-In là BKD nhưng **BKD1 không được lấy làm nguồn** (chỉ BKD2/BKD3). Đây là lý do dòng In-In ở bảng trên chỉ liệt kê BKD2/BKD3, còn In-Ex (đích là kho ngoài) mới dùng BKD1 ở bậc dự phòng. Cụm là *phạm vi kho có thể xét*; mỗi luồng khai đúng kho của cụm được làm nguồn (đã loại sẵn kho đích).

> **"Cụm kho nguồn" trỏ tới một *Cụm* (đã khai ở mục 2.1):** các giá trị cột *Cụm kho nguồn* (**BKD**, **NKD**) là **Cụm** định nghĩa ở mục **2.1. Danh mục kho** (Cụm BKD = BKD1/BKD2/BKD3; Cụm NKD = {NKD}). Đây cũng là **tập giá trị hợp lệ** cho cột *Cụm kho nguồn* trong file kế hoạch (mục **3.3**). Bảng 2.4 này **không định nghĩa lại thành viên cụm** — nó chỉ khai **trong từng luồng, cụm đó lấy theo bậc nào** (ví dụ In-In không dùng BKD1 làm nguồn; In-Ex dùng BKD1 ở bậc dự phòng).

> **Cách hệ thống dùng cấu hình này (chi tiết ở mục 4.2):** với một dòng kế hoạch, hệ thống lấy đúng các bậc theo (luồng × cụm kho nguồn), rồi xét **từ bậc nhỏ đến lớn**: ở mỗi bậc, hệ thống **xét chung lô của tất cả các kho trong bậc** — xếp toàn bộ lô theo chiến thuật (FEFO/LEFO/…) rồi chọn từ trên xuống, không lấy cạn kho này mới sang kho kia; nếu các kho ở bậc đó hết mà chưa đủ số pallet thì mới sang bậc kế. Ví dụ In-Ex từ BKD: lấy hết hàng BKD2 + BKD3 (bậc 1) trước, thiếu mới chạm BKD1 (bậc 2). Khi chạm bậc dự phòng BKD1, hệ thống kích hoạt **cổng duyệt kiểm soát hạn dùng (break-FEFO)** — xem mục **4.2**. _(Truy vết: BR-004, BR-005, BR-029, BR-030, BRULE-01/04.)_
>
...
_[Chèn ảnh: bảng cấu hình theo luồng × cụm kho nguồn, mỗi bậc lấy với các kho lấy ở bậc đó]_
...
**Bước 4 — Nhận diện luồng & nhánh.** Hệ thống xác định từng dòng thuộc **In-In** hay **In-Ex**, và trong In-Ex là **nhánh BKD** hay **nhánh kho NKD** (căn cứ cột *Luồng* và *Cụm kho nguồn*) để áp đúng cơ chế nguồn lấy hàng ở mục **4.2**.
...
## 4.1. Phân bổ batch theo chiến thuật phân bổ

Nghiệp vụ này trả lời câu hỏi **"chọn lô nào trong số hàng đang có"**: với mỗi dòng kế hoạch, hệ thống chọn các lô (batch) cụ thể trong tồn của các kho nguồn để đáp ứng số pallet yêu cầu, theo đúng chiến thuật mà Planner MDLZ đặt cho từng mã hàng. Việc *lấy hàng ở kho nào, xét chung những kho nào* thuộc mục **4.2. Phân bổ theo luồng & nguồn lấy hàng** — mục **4.2** xác định *bậc nguồn* (các kho được xét chung ở mỗi bậc, thứ tự bậc dự phòng theo cấu hình mục **2.4**) và điều phối vòng lặp qua các bậc. Mục **4.1** này nhận tồn của **một bậc** làm đầu vào và tập trung vào quy tắc chọn lô **trong bậc đó**.

### 4.1.1. Bảng tổng hợp

| Hạng mục | Nội dung |
|---|---|
| Mục đích | Chọn lô cụ thể trong tồn các kho nguồn theo chiến thuật của từng SKU, thay thao tác dò lô thủ công bằng Excel |
| Nhân sự chịu trách nhiệm | Hệ thống tự phân bổ; Planner MDLZ đặt chiến thuật cho từng SKU trong file kế hoạch |
| Hệ thống thao tác | Module — chức năng *Phân bổ batch* |
| Hệ thống tích hợp | WMS (lấy tồn theo lô qua API: `AVAILABLE`, `LOTTABLE01`, `SHELF LIFE (%)`, `STATUS`) |
| Tần suất | Mỗi lần phân bổ một kế hoạch (hằng ngày) |
| Đầu vào | Dòng kế hoạch hợp lệ (SKU, số pallet, chiến thuật) + tồn theo lô của các kho nguồn **thuộc bậc đang xét** (bậc nguồn & thứ tự do mục 4.2 xác định) |
| Đầu ra | Danh sách phân bổ: mỗi dòng kế hoạch → một/nhiều dòng (lô, kho, số pallet) |

### 4.1.2. Mục đích

Hiện trạng, mỗi ngày nhân viên kho phải tải dữ liệu tồn về Excel, lọc tay theo hạn dùng rồi dò chọn lô nào xuất trước — tốn công, dễ sai và phụ thuộc vào kinh nghiệm một cá nhân; cùng một tình huống nhưng hai người có thể chọn lô khác nhau. Nghiệp vụ này để **hệ thống tự chọn lô theo chiến thuật đặt sẵn cho từng SKU**, cho kết quả nhất quán và truy được lý do chọn từng lô, giải phóng nhân viên khỏi thao tác dò tay.

### 4.1.3. Danh sách tính năng

| STT | Tên tính năng | Mô tả |
|---|---|---|
| 1 | Đặt chiến thuật theo từng SKU | Mỗi dòng kế hoạch mang một chiến thuật riêng: FEFO / LEFO / Nhập xưởng / Chỉ định batch |
| 2 | Quy đổi số lượng | Nở số pallet kế hoạch sang đơn vị nhỏ nhất bằng hệ số quy đổi master SKU (mục 2.2) |
| 3 | Lấy tồn theo lô | Lấy tồn khả dụng theo lô của các kho nguồn **thuộc bậc đang xét** (bậc do mục 4.2 xác định) từ WMS (kèm % hạn dùng, trạng thái) tại thời điểm phân bổ |
| 4 | Giữ & trừ tồn đã phân bổ | Tồn khả dụng của mỗi lô bị **trừ phần đã phân bổ (đã giữ)** — chống trùng cả **giữa các dòng trong một kế hoạch** lẫn **giữa các kế hoạch chưa tích hợp xuống WMS**; phần giữ nhả khi đơn đã tích hợp hoặc phương án bị hủy/mở khóa |
| 5 | Lọc trạng thái | Giữ lô có `STATUS` thuộc danh sách cho phép của kho (mục 2.1 Bảng 2); kho không khai thì giữ tất cả |
| 6 | Sắp xếp & chọn lô theo chiến thuật | Xếp lô theo tiêu chí của chiến thuật rồi chọn từ trên xuống |
| 7 | Lấy cuốn chiếu theo lô | Lấy trọn lô hiện tại rồi sang lô kế, tuần tự cho đến khi đủ số lượng |
| 8 | Cảnh báo | Báo khi lô chỉ định không đủ, hoặc các kho nguồn không đủ đáp ứng kế hoạch |

### 4.1.4. Mô tả dữ liệu đầu ra

Đầu ra của nghiệp vụ là **danh sách phân bổ**: mỗi dòng kế hoạch được tách thành một hoặc nhiều dòng phân bổ, mỗi dòng chỉ rõ lấy lô nào, ở kho nào, bao nhiêu pallet.

**Lưu đồ chọn lô theo chiến thuật:**
...
flowchart TD
    ...
    A1 --> A2["Lấy tồn theo lô từ WMS cho các kho thuộc bậc đang xét (bậc do mục 4.2 xác định): AVAILABLE, LOTTABLE01, SHELF LIFE %, STATUS"]
...
> **Bước 6 — Kiểm tra đủ.** Nếu các kho của bậc đang xét không đủ, **mục 4.2 chuyển sang bậc nguồn kế tiếp** (lặp lại bước 2–5 cho bậc mới); khi đã hết bậc mà vẫn thiếu thì đưa vào xử lý ngoại lệ thiếu tồn (mục 6).
...
| Cờ cảnh báo | thiếu tồn / lô chỉ định không đủ; **break-FEFO do mục 4.2 gắn khi lô BKD1 dự phòng cũ hơn nhóm BKD2/BKD3** (nếu có) | — |
...
Tình huống: dòng kế hoạch SKU 4299263, **chiến thuật FEFO**, cần **16 pallet**, lấy chung BKD2 + BKD3 (mục 4.2). Quy cách: số lượng quy đổi cấp pallet = 540 PCS/pallet. Sau khi lọc trạng thái, tồn các kho nguồn có ba lô:
...
**Bước 1 — Xem kết quả phân bổ.** Sau khi kế hoạch hợp lệ, hệ thống tự phân bổ và hiển thị kết quả theo từng dòng kế hoạch: mỗi dòng bung ra các lô được chọn kèm kho, số pallet, % hạn dùng và cờ cảnh báo (nếu có). Người dùng đọc được vì sao mỗi lô được chọn (theo chiến thuật nào).

_[[MH-4.1: Màn hình kết quả phân bổ batch theo chiến thuật]]_
_[Chèn ảnh: bảng kết quả phân bổ — dòng kế hoạch bung theo lô, cột kho/số pallet/% hạn dùng/cờ; bộ lọc theo luồng & chiến thuật]_

**Bước 2 — Xử lý nguồn theo luồng.** Kết quả chọn lô được chuyển sang **mục 4.2** để dựng nguồn theo luồng và duyệt break-FEFO (nếu chạm BKD1 hạn thấp) trước khi tạo đơn.
...
## 4.2. Phân bổ theo luồng & nguồn lấy hàng

Nghiệp vụ này trả lời câu hỏi **"lấy hàng ở kho nào, theo thứ tự nào"**. Theo cấu hình nguồn lấy hàng của từng luồng (mục **2.4**), hệ thống lấy lô lần lượt **qua từng bậc kho** cho đến khi đủ số pallet — việc chọn lô trong mỗi bậc do mục **4.1** đảm nhận. Khi phải chạm kho xuất bán **BKD1**, hệ thống thêm **cổng duyệt kiểm soát hạn dùng**.

Ba kịch bản nguồn theo luồng (theo cấu hình mặc định ở mục **2.4**):
...
| Hạng mục | Nội dung |
|---|---|
| Mục đích | Dựng nguồn lấy hàng theo luồng & duyệt theo bậc; kiểm soát khi buộc phải chạm kho xuất bán BKD1 |
| Nhân sự chịu trách nhiệm | Hệ thống tự dựng nguồn & duyệt bậc; Planner MDLZ duyệt cổng hạn dùng khi lô BKD1 cũ hơn nhóm BKD2/BKD3 (vai trò ở mục 7) |
| Hệ thống thao tác | Module — chức năng *Phân bổ batch* (cùng màn hình với mục 4.1) |
| Hệ thống tích hợp | WMS (lấy tồn theo lô của các kho trong bậc qua API) |
| Tần suất | Mỗi lần phân bổ một kế hoạch (hằng ngày) |
| Đầu vào | Dòng kế hoạch đã gắn luồng/nhánh (mục 3.4) + cấu hình nguồn theo luồng (mục 2.4) + tồn theo lô từ WMS |
| Đầu ra | Danh sách phân bổ hoàn chỉnh (gộp các bậc), kèm bậc lấy & cờ break-FEFO |

### 4.2.2. Mục đích

Hiện trạng, quy tắc *"lấy kho nào trước, kho nào lấy chung"* nằm trong trí nhớ nhân viên kho ("ưu tiên BKD2/BKD3, thiếu mới lấy BKD1"). Khi buộc phải lấy từ BKD1 — vốn là **kho xuất bán chính** — lại không có bước kiểm soát, dễ đụng vào hàng đang để bán hoặc phá nguyên tắc hạn dùng mà không ai duyệt. Nghiệp vụ này để hệ thống **dựng nguồn theo cấu hình tường minh** (mục 2.4), tự duyệt bậc và chỉ chạm BKD1 khi thật sự thiếu, kèm **cổng duyệt kiểm soát hạn dùng** — cho kết quả nhất quán, có kiểm soát và truy được vì sao lấy ở bậc/kho nào.

### 4.2.3. Danh sách tính năng

| STT | Tên tính năng | Mô tả |
|---|---|---|
| 1 | Tra cấu hình nguồn theo luồng | Lấy danh sách bậc & các kho mỗi bậc theo (luồng × cụm kho nguồn) từ cấu hình mục 2.4 |
| 2 | Loại kho đích khỏi nguồn | Bỏ kho đích (cột *Kho đích* của kế hoạch) ra khỏi tập kho nguồn — vd In-In đích BKD1 nên BKD1 không làm nguồn |
| 3 | Gộp tồn theo bậc (xét chung) | Gộp tồn của tất cả kho trong cùng một bậc thành một danh sách chung để xét chung, không lấy cạn kho này mới sang kho kia |
| 4 | Điều phối vòng lặp bậc | Duyệt bậc từ nhỏ đến lớn; mỗi bậc gọi quy trình chọn lô (mục 4.1); thiếu thì sang bậc kế |
| 5 | Cổng duyệt kiểm soát hạn dùng (break-FEFO) | Khi lô lấy từ bậc dự phòng BKD1 (In-Ex BKD) **cũ hơn (≤ % hạn dùng) nhóm BKD2/BKD3** và cổng đang bật (mục 2.3), yêu cầu Planner MDLZ xác nhận trước khi phân bổ lô BKD1 |
| 6 | Gắn cờ & cảnh báo | Gắn cờ break-FEFO lên dòng lấy từ BKD1 cũ hơn nhóm BKD2/BKD3; cảnh báo khi đã hết bậc mà vẫn thiếu tồn |

### 4.2.4. Mô tả dữ liệu đầu ra

Đầu ra của nghiệp vụ là **danh sách phân bổ hoàn chỉnh** của dòng kế hoạch — gộp kết quả chọn lô qua tất cả các bậc đã chạm — mỗi dòng chỉ rõ lấy lô nào, ở kho nào, thuộc bậc nào, bao nhiêu pallet, kèm cờ break-FEFO nếu lấy từ BKD1 dự phòng.

**Lưu đồ dựng nguồn & duyệt theo bậc:**
...
> **Bước 1 — Tra cấu hình nguồn & loại kho đích.** Theo (luồng × cụm kho nguồn) của dòng kế hoạch, hệ thống lấy danh sách **bậc lấy** (theo thứ tự) và các **kho xét chung** ở mỗi bậc từ cấu hình mục **2.4**; đồng thời loại **kho đích** khỏi tập kho nguồn (một kho không thể vừa là nguồn vừa là đích — vd In-In đích BKD1 nên BKD1 không làm nguồn).

> **Bước 2 — Duyệt từng bậc.** Xét bậc **từ nhỏ đến lớn**: ở mỗi bậc gộp tồn các kho thành một danh sách chung rồi chọn lô theo chiến thuật (quy trình mục **4.1**); thiếu thì sang bậc kế. **Hết bậc mà vẫn thiếu** → đưa sang bước *chọn lô khác / xử lý thiếu tồn*.
...
> **Bước 4 — Gắn cờ & chờ duyệt.** Hệ thống gắn cờ **break-FEFO** để truy vết và chờ **Planner MDLZ** duyệt. _(Cổng duyệt bật/tắt được ở mục **2.3**; nếu tắt thì bỏ qua bước chờ, phân bổ thẳng nhưng vẫn giữ cờ.)_

> **Bước 5 — Được duyệt?** Nếu **được duyệt** (hoặc cổng tắt) → **phân bổ hoàn chỉnh**. Nếu **từ chối** → sang bước **chọn lô khác / xử lý thiếu tồn**: Planner chọn tay lô thay; không có lô phù hợp thì coi như thiếu tồn (ngoại lệ mục **6**).
...
| Kho lấy | Kho nguồn của lô | BKD1 |
| Mã lô (batch) | Lô được chọn (`LOTTABLE01`) | L2026-02-10 |
| Số pallet phân bổ | Số pallet lấy từ lô này | 6 |
| Cờ break-FEFO | Bật khi lô lấy từ BKD1 dự phòng cũ hơn nhóm BKD2/BKD3 (chờ duyệt / đã duyệt) | Có — chờ duyệt |

### 4.2.5. Ví dụ mô phỏng

Tình huống: dòng kế hoạch **In-Ex**, cụm nguồn **BKD**, đích kho ngoài **BEE**, SKU 4299263, **chiến thuật FEFO**, cần **20 pallet**. Cấu hình mục 2.4: bậc 1 = **BKD2 + BKD3** (xét chung), bậc 2 (dự phòng) = **BKD1**. Cổng duyệt kiểm soát hạn dùng: **Bật**. Sau khi lọc trạng thái, tồn các bậc:
...
### 4.2.6. Quy trình thao tác trên hệ thống

**Đường dẫn:** Module → *Phân bổ* → kết quả phân bổ của kế hoạch vừa tiếp nhận (nối tiếp mục 4.1).

**Bước 1 — Xem phân bổ theo bậc & nguồn.** Hệ thống hiển thị mỗi dòng phân bổ kèm **bậc lấy**, kho nguồn, lô và cờ break-FEFO; người dùng thấy rõ phần nào lấy từ bậc chính, phần nào chạm bậc dự phòng BKD1.

_[[MH-4.2: Màn hình phân bổ theo luồng & nguồn lấy hàng]]_
_[Chèn ảnh: bảng phân bổ nhóm theo bậc, cột kho/lô/số pallet/cờ break-FEFO; bộ lọc theo luồng & nhánh]_

**Bước 2 — Duyệt cổng kiểm soát hạn dùng (nếu có).** Với các dòng mang cờ break-FEFO (lô BKD1 cũ hơn nhóm BKD2/BKD3), Planner MDLZ (mục **7**) xem đối chiếu hạn dùng lô BKD1 với tồn bậc trên rồi **xác nhận** hoặc **từ chối**; nếu **từ chối**, Planner MDLZ **chọn tay lô khác** để thay — không có lô phù hợp thì phần đó coi như thiếu tồn (mục **6**).
...
**Bước 3 — Chuyển sang tạo đơn.** Danh sách phân bổ hoàn chỉnh (đã qua duyệt cổng nếu có) được chuyển sang mục **5.1** để tạo & tích hợp đơn.

> **Khi cổng break-FEFO bị từ chối:** người có thẩm quyền (phân quyền ở mục **7**) được **chọn tay lô khác** để thay ngay tại khâu duyệt break-FEFO (mục **4.2**); nếu không có lô phù hợp để thay thì phần đó mới coi như thiếu tồn (ngoại lệ mục **6**). _(Truy vết: BR-004, BR-005, BR-029, BR-030; BRULE-01, BRULE-04.)_
...
|---|---|---|---|---|---|
| **EX-01** | **Thiếu tồn** — các bậc nguồn không đủ số pallet kế hoạch | Phân bổ (mục **4.1–4.2**) | Chặn dòng | Phân bổ tối đa phần có tồn theo các bậc nguồn; phần thiếu gắn cảnh báo *thiếu tồn*, hiển thị ở khâu phân bổ (mục **4**) để người dùng giảm số pallet về phần đủ và chuyển phần thiếu sang xử lý ngoài — **không tự ý lấy quá tồn** | Admin FML |
...
| **EX-04** | **Từ chối duyệt hạn dùng (break-FEFO)** | Duyệt break-FEFO (mục **4.2**) | Chặn dòng | Khi người có thẩm quyền từ chối lấy hàng BKD1 dự phòng hạn thấp → người có thẩm quyền (mục **7**) **chọn tay lô khác** để thay; nếu không có lô phù hợp thì phần đó **coi như thiếu tồn** (xử lý như EX-01) | Planner MDLZ |
...
> **Ranh giới với cảnh báo nghiệp vụ:** *break-FEFO chờ duyệt* (lấy BKD1 dự phòng hạn thấp) bản thân là **cảnh báo** xử lý ngay tại khâu duyệt (mục **4.2**); chỉ khi **bị từ chối** mới chuyển thành ngoại lệ EX-04. Tương tự, *thiếu tồn* được phát hiện và quyết ngay tại khâu phân bổ — người dùng chủ động chọn phần đủ và tách phần thiếu ra. _(Truy vết: BR-006, BR-012, BR-029; NFR-03, NFR-05.)_
...
| Duyệt cổng hạn dùng (break-FEFO) | **4.2** | Planner MDLZ | Có (tắt ⇒ bỏ cổng duyệt) |
...
| **OQ-09** | Mã kho & `STATUS` khả dụng của kho **NKD** | **2.1**, **4.2** | Do quản trị **tự thiết lập** trên màn cấu hình (không ảnh hưởng thiết kế) |
...
| **OQ-19** | Khi cổng **break-FEFO bị từ chối** | **4.2**, **6** | Người có thẩm quyền (mục **7**) **chọn tay lô khác**; không có lô thay thì coi như thiếu tồn |
